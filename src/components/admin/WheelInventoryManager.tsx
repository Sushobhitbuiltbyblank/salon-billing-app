...
interface WheelInventoryManagerProps {
  standalone?: boolean;
}

export function WheelInventoryManager({ standalone = false }: WheelInventoryManagerProps) {
  const {
    wheelInventory,
  ...
  const stats = useMemo(() => {
    ...
    const totalUnits = wheelInventory.reduce((acc, item) => acc + (item.quantity || 0), 0);
  ...
  return (
    <div className="space-y-4">
      {/* STANDALONE HEADER (IF ACCESSED DIRECTLY VIA /admin/wheel-inventory) */}
      {standalone && (
        <header className="sticky top-0 z-40 w-full border-b border-zinc-800/80 bg-zinc-950/90 backdrop-blur-xl px-4 py-3 -mx-4 -mt-4 mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-400">
                  Independent Reward Stock Pool • {settings.salon_name || "Belezia Salon"}
                </p>
              </div>
            </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-zinc-900/60 p-4 rounded-2xl border border-zinc-800">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                <span>Lucky Wheel Pool Stocks (Independent Reward Inventory)</span>
              </h3>
              <Badge variant="outline" className="bg-purple-500/15 text-purple-300 border-purple-500/30 text-[10px] font-mono">
                {wheelInventory.length} Items Configured
              </Badge>
            </div>
            <p className="text-xs text-zinc-400 mt-1">
              Live reward pool for the lucky wheel. When customers win, stocks auto-decrement here in real-time.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              variant="outline"
              size="sm"
              onClick={handleResetDefaultPool}
              className="gap-1 text-xs border-zinc-700 text-zinc-300 hover:bg-zinc-800"
              title="Reset default 6 Belezia reward items"
            >
      ...
      {/* TOAST / NOTIFICATION */}
      {notification && (
        <div className="p-3 rounded-xl bg-emerald-950/60 border border-emerald-500/40 text-emerald-300 text-xs font-bold flex items-center gap-2 shadow-lg animate-in fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
        <Card className="p-4 bg-zinc-900/70 border-zinc-800">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-[11px] text-zinc-500">Configured Items</span>
          </div>
        </Card>
        ...
        >
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-[11px] text-zinc-500">Needs Refill</span>
          </div>
        </Card>
      ...
      {/* 2. CONTROLS BAR: SEARCH, STATUS & ADD */}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {standalone && (
            <Button
              ...
              size="sm"
              onClick={handleResetDefaultPool}
              className="gap-1.5 text-xs border-zinc-800 text-zinc-300 hover:bg-zinc-800"
            >
              <RotateCcw className="h-3.5 w-3.5" />
              <span>Reset 6 Defaults</span>
            </Button>
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1">
        ...
        >
          All Pool Items ({wheelInventory.length})
        </button>
              ...
              {filteredItems.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-zinc-500 text-xs">
                    No reward pool stocks match the selected filters.
                  </td>
                </tr>
              ) : (
                    ...
                    >
                      ...
                      <td className="p-3.5 text-center">
                        <div className="inline-flex flex-col items-center">
                          <span
                            className={`text-base font-mono font-black px-2.5 py-0.5 rounded-lg border ${
                              isOutOfStock
